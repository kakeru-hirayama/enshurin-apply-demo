...
var API = (function () {
   ...
   *
   * ★貼ってある申込からも外す。
   *   外さないと、名前の分からないふせんが残り続ける。
   *
   *   使わなくなっただけなら active を false にする手もあるが、
   *   一覧に出ないものが裏に溜まっていくほうが分かりにくい。
   *   消すときは、すっかり消す。
   */
  function removeTag(tagId, staffId) {
    var t = db.findByKey('M_TAG', tagId);
    if (!t) return { ok: false, error: 'ふせんが見つかりません' };
    ...
    // 貼ってあるものを、先に全部はがす
    var n = 0;
    db.readAll('T_APP_TAG')
      .filter(function (r) { return r.tag_id === tagId; })
      .forEach(function (r) {
        db.remove('T_APP_TAG', [r.app_id, r.tag_id]);
        n++;
      });

    db.remove('M_TAG', tagId);
    writeAudit(staffId, 'ふせんを消した', tagId,
               t.name + '（' + n + '件からはがした）');
   ...
   *
   * ★黙って上書きしない。
   *   上書きすると、変えたご本人も、消された方も気づけない。
   *
   * @param {string} appId
   * @param {string} seenAt  画面を開いたときの updated_at
   * @return {Object} { ok, changed, by, at }
   */
  function checkConflict(appId, seenAt) {
    var a = db.findByKey('T_APPLICATION', appId);
    if (!a) return { ok: false, error: '申込が見つかりません' };

    // 画面が古い更新時刻を持っていなければ、確かめようがない
    if (!seenAt) return { ok: true, changed: false };

    if (String(a.updated_at || '') === String(seenAt)) {
      return { ok: true, changed: false };
    }

    // 誰が変えたのかを、記録から探す
    var by = '';
    var logs = db.readAll('T_AUDIT')
      .filter(function (x) { return x.target === appId; })
      .sort(function (x, y) { return String(y.at) < String(x.at) ? -1 : 1; });
    if (logs.length) {
      var st = db.findByKey('M_STAFF', logs[0].staff_id);
      by = st ? st.name : (logs[0].staff_id || '');
    }

    return {
      ok: true, changed: true,
      by: by, at: a.updated_at,
      what: logs.length ? logs[0].action : ''
    };
  }
   ...
   */
  function updateStatus(appId, newStatus, staffId, note, seenAt) {
    var a = db.findByKey('T_APPLICATION', appId);
    if (!a) return { ok: false, error: '申込が見つかりません' };

    // ★ほかの方が先に変えていないかを見る。
    //   seenAt を渡さなければ確かめない（これまでどおり動く）
    if (seenAt) {
      var c = checkConflict(appId, seenAt);
      if (c.changed) {
        return {
          ok: false, conflict: true,
          error: 'ほかの方が先に変更されています' +
                 (c.by ? '（' + c.by + ' さん）' : '') +
